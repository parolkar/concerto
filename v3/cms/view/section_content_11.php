...
$description = Language::string(520);
if ($section != null) {
    $description.=" " . Language::string(521) . ":<hr/>" . $section->get_description();
}
?>
        ...
        <tr>
            ...
            <td class="tdSectionColumnAction">
                <table class="fullWidth">
                    <tr>
                        <td>
                            <span class="spanIcon ui-icon ui-icon-plus tooltip" title="<?= Language::string(205) ?>" onclick="Test.uiAddNewRelatedObject(11)"></span>
                        </td>
                        ...
                        <?php if (isset($vals[0]) && $vals[0] != 0) { ?>
                            <td>
                                <span class="spanIcon ui-icon ui-icon-extlink tooltip" title="<?= Language::string(522) ?>" onclick="Test.uiGoToRelatedObject(11,<?= $vals[0] ?>)"></span>
                            </td>
                        <?php } ?>
                        <td class="fullWidth">
                            <select id="selectTest_<?= $_POST['counter'] ?>" class="fullWidth ui-widget-content ui-corner-all fullWidth" onchange="Test.uiRefreshSectionContent(<?= $_POST['type'] ?>, <?= $_POST['counter'] ?>, Test.getSectionValues(Test.sectionDivToObject($('#divSection_<?= $_POST['counter'] ?>'))))">
                                <option value="0">&lt;<?= Language::string(73) ?>&gt;</option>
                                <?php
                                $sql = $logged_user->mysql_list_rights_filter("Test", "`name` ASC");
                                $z = mysql_query($sql);
                                while ($r = mysql_fetch_array($z)) {
                                    $t = Test::from_mysql_id($r[0]);
                                    ?>
