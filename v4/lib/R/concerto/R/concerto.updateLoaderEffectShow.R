concerto.updateLoaderEffectShow <-
function(effectShow) {
  ...
  dbSendQuery(concerto$db$connection, statement = sprintf("UPDATE `%s`.`TestSession` SET `loader_effect_show` = '%s' WHERE `id`=%s",dbName,effectShow,sessionID))
}
