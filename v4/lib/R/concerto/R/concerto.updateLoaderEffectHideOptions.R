concerto.updateLoaderEffectHideOptions <-
function(effectHideOptions) {
  ...
  dbSendQuery(concerto$db$connection, statement = sprintf("UPDATE `%s`.`TestSession` SET `loader_effect_hide_options` = '%s' WHERE `id`=%s",dbName,effectHideOptions,sessionID))
}
