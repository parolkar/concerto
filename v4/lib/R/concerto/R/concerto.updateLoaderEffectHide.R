concerto.updateLoaderEffectHide <-
function(effectHide) {
  ...
  dbSendQuery(concerto$db$connection, statement = sprintf("UPDATE `%s`.`TestSession` SET `loader_effect_hide` = '%s' WHERE `id`=%s",dbName,effectHide,sessionID))
}
