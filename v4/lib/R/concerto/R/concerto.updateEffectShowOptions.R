concerto.updateEffectShowOptions <-
function(effectShowOptions) {
  ...
  dbSendQuery(concerto$db$connection, statement = sprintf("UPDATE `%s`.`TestSession` SET `effect_show_options` = '%s' WHERE `id`=%s",dbName,effectShowOptions,sessionID))
}
